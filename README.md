...
flowchart LR
  ...
  C --> D[Baseline (selected model)<br/>gpt-5.2 or gpt-5-mini]
  C --> D2[Baseline (alternate model)<br/>the other model]
  C --> E[Juror A/B/C<br/>selected model]
  E --> F[Critique Agents]
  F --> R[Rebuttal Agents<br/>deep deliberation only]
  R --> G[Revision Agents]
  F -->|standard mode| G
  C --> H[Chief Justice<br/>selected model]
  C --> I[Evaluator<br/>selected model]
